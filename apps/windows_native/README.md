...
- 共享 core 已完成 `fieldReference` 九态单跳解析、目标字段生命周期保护、分类改名传播、安全搜索投影、copy-import 条目 ID 重映射和同步冲突保真；旧数据空字段 ID 名称回退与非空 ID 精确匹配规则在各端一致。Windows CLI 的字段级关系展示仍留在 P12，不提前暴露目标字段值或原始 ID。
