...
import com.example.passwordmanagernative.model.safeExportName
import java.io.File
...
class FileVaultRepository(private val baseDirectory: File) {
    ...
    fun saveEntryExport(entry: VaultEntry, at: Instant = Instant.now()): File {
        ...
        exportFile.writeText(
            VaultJson.encodeScopedExport(
                ScopedVaultExport(
                    scope = com.example.passwordmanagernative.model.ScopedExportScope.ITEM,
                    exportedAt = at,
                    item = entry,
                    category = null,
                    items = null,
    ...
    fun saveCategoryExport(category: String, entries: List<VaultEntry>, at: Instant = Instant.now()): File {
        ...
        exportFile.writeText(
            VaultJson.encodeScopedExport(
                ScopedVaultExport(
                    scope = com.example.passwordmanagernative.model.ScopedExportScope.CATEGORY,
                    exportedAt = at,
                    item = null,
                    category = category,
                    items = entries,
