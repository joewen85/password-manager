...
- 字段关联专项测试已覆盖模板编辑、候选过滤、五态详情以及原始 ID/目标秘密隔离
...
1. 创建两个分类，例如“服务器”和“账号”，在“服务器”分类新增“关联条目”字段。
2. 分别选择“账号”和“不限分类”作为目标分类，保存后重新进入分类编辑，确认字段 ID、类型和目标分类不丢失。
3. 创建账号目标条目和服务器来源条目，确认选择器只显示未删除且符合目标分类的候选；选择、更换和清空都能正常保存。
4. 分别验证未选择、正常、目标不存在、目标已删除、目标分类不匹配五态；确认详情提供查看目标、修复或清除操作。
5. 删除配置中的目标分类，确认原目标分类文本仍显示并保留，不被改写成“未分类”。
6. 导入包含未知 `valueType` 的模板，确认该字段只读显示，保存其他字段后未知字段的 ID、名称、类型和目标分类仍保持不变。
7. 用存储的引用 ID 和目标条目的密码、Token、Secret、备注执行搜索，确认列表摘要、详情、搜索结果和剪贴板不出现这些值。
8. 锁定、杀进程并重启后重新解锁，确认引用值和五态解析保持一致。
9. 执行单条/分类导出，确认不会因引用自动携带目标条目；执行同批复制导入时确认批内引用正确重映射。
