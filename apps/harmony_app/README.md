...
- 字段关联 P1 数据契约已接入 ArkTS 类型、持久化、同步和条目/分类导入导出，能够无损保留模板类型、目标分类、模板字段 ID 和稳定目标条目 ID；标签职责不变。当前不提供关联编辑 UI，格式与上线顺序见 `../../docs/FIELD_REFERENCE_CONTRACT.md`。
...
- The P1 entry-reference data contract is preserved by ArkTS types, persistence, sync, and item/category import/export, including template type, target category, template-field ID, and stable target-entry ID. Tags remain unchanged. Reference editing UI is not exposed yet; see `../../docs/FIELD_REFERENCE_CONTRACT.md` for the format and rollout order.
