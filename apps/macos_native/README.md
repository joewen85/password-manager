...
- P7 已加入字段级关联的数据契约：`FieldTemplate.targetFieldId` 保存不透明的目标模板字段 ID，缺失时默认读取为空字符串，并在完整快照、范围导入导出和同步 payload 中无损保留。当前阶段不提供 `fieldReference` 的 macOS UI 或解析行为，也不改变既有 `entryReference` 语义；这是现有加密 JSON 内的加法字段，不需要数据库迁移、权限或 entitlement 变更。
...
- P7 adds the field-level reference data contract. `FieldTemplate.targetFieldId` stores an opaque target template-field ID, defaults to an empty string when absent, and is preserved losslessly through full snapshots, scoped import/export, and sync payloads. This phase adds no macOS UI or resolver behavior for `fieldReference` and does not change existing `entryReference` semantics. It is an additive field inside the existing encrypted JSON and requires no database migration, permission, or entitlement change.
