...
struct FieldReferenceContractTests {
    ...
    func fieldReferenceMetadataRoundTrips() throws {
        ...
        let encoder = JSONEncoder.scopedContractEncoder
    ...
    func sharedContractFixturesRoundTripSemantically() throws {
        ...
        for name in ["scoped-item-entry-reference.json", "scoped-category-entry-reference.json"] {
            let scopedExport = try repository.decodeScopedExport(fixtureData(name))
            ...
            let encoded = try JSONEncoder.scopedContractEncoder.encode(scopedExport)
            #expect(try repository.decodeScopedExport(encoded) == scopedExport)
        }
    }
    ...
    func opaqueIdentifiersSurviveSnapshotRoundTrip() throws {
        let repository = FileVaultRepository(
            baseDirectory: FileManager.default.temporaryDirectory
                .appendingPathComponent("PasswordManagerMacOSOpaqueContract-\(UUID().uuidString)", isDirectory: true)
        )
    ...
    func scopedExportsIncludeOnlySourceCategoryTemplate() throws {
        let repository = FileVaultRepository(
            baseDirectory: FileManager.default.temporaryDirectory
                .appendingPathComponent("PasswordManagerMacOSScopedContract-\(UUID().uuidString)", isDirectory: true)
        )
        ...
        let exportFile = try repository.makeEntryExport(
            entry,
            categoryTemplates: [sourceTemplate, targetTemplate],
            at: Date(timeIntervalSince1970: 1_700_000_000)
        )
        let export = try repository.decodeScopedExport(exportFile.data)
        ...
        let categoryExportFile = try repository.makeCategoryExport(
            category: "Servers",
            entries: [entry],
            categoryTemplates: [sourceTemplate, targetTemplate],
            at: Date(timeIntervalSince1970: 1_700_000_001)
        )
        let categoryExport = try repository.decodeScopedExport(categoryExportFile.data)
    ...
    func versionOneScopedExportsRemainReadable() throws {
        let repository = FileVaultRepository(
            baseDirectory: FileManager.default.temporaryDirectory
                .appendingPathComponent("PasswordManagerMacOSV1Contract-\(UUID().uuidString)", isDirectory: true)
        )
        let export = try repository.decodeScopedExport(
            Data(
                #"""
                {
                  "version":1,
    ...
    func scopedImportMergesTemplatesAndPreservesReferences() throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("PasswordManagerMacOSScopedImportContract-\(UUID().uuidString)", isDirectory: true)
    ...
    func scopedCopyImportRemapsReferencesToCopiedTargets() throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("PasswordManagerMacOSReferenceRemapTests-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: directory) }
        let repository = FileVaultRepository(baseDirectory: directory)
        let store = VaultStore(repository: repository, syncSettingsRepository: nil)
        #expect(store.setupMasterPassword("test-password", confirmation: "test-password"))
    ...
    func scopedOverwriteImportRemapsReferencesToExistingDestinationID() throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("PasswordManagerMacOSExistingReferenceRemapTests-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: directory) }
        let repository = FileVaultRepository(baseDirectory: directory)
        let store = VaultStore(repository: repository, syncSettingsRepository: nil)
        #expect(store.setupMasterPassword("test-password", confirmation: "test-password"))
...
private extension JSONEncoder {
    static var scopedContractEncoder: JSONEncoder {
        let encoder = JSONEncoder()
        encoder.dateEncodingStrategy = .iso8601
        return encoder
    }
