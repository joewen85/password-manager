...
struct FieldLevelReferenceBehaviorTests {
    ...
    @MainActor
    @Test("Store copy import, search, and template dependency guard are integrated")
    func storeIntegration() throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("PasswordManageriOSFieldReferenceP8-\(UUID().uuidString)", isDirectory: true)
        ...
        let before = try #require(store.categoryTemplate(for: "Accounts"))
        #expect(!store.updateCategoryTemplate(category: "Accounts", requestedCustomFields: []))
        #expect(store.categoryTemplate(for: "Accounts") == before)
        ...
        #expect(store.updateCategoryTemplate(
            category: "Accounts",
            requestedCustomFields: [renamed]
        ))
        #expect(store.categoryTemplate(for: "Accounts")?.fields.first {
            $0.id == targetField.id
        }?.name == "Login name")
    }
}
