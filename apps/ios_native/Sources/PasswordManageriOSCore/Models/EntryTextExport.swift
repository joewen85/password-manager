...
extension VaultEntry {
    ...
    private func exportValue(
        ...
        case "tags": return payload.tags.joined(separator: ", ")
        default: break
        }
    ...
    private func referencedEntryText(_ id: String?, entries: [VaultEntry]) -> String {
        ...
        guard !target.isDeleted else { return "Referenced entry was deleted." }
        return target.label.isEmpty ? "Untitled" : target.label
    }
}
...
private func entryReferenceExportText(_ resolution: EntryReferenceResolution) -> String {
    ...
    case .empty: return "No entry selected."
    case .resolved:
        guard let target = resolution.target else { return "Referenced entry is unavailable." }
        return target.label.isEmpty ? "Untitled" : target.label
    case .missing: return "Referenced entry is unavailable."
    case .deleted: return "Referenced entry was deleted."
    case .categoryMismatch: return "Referenced entry is outside the target category."
    }
}
...
private func fieldReferenceExportText(_ resolution: FieldReferenceResolution) -> String {
    switch resolution.status {
    case .empty: return "No entry selected."
    case .invalidConfiguration: return "Field reference configuration needs repair."
    case .missing: return "Referenced entry is unavailable."
    case .deleted: return "Referenced entry was deleted."
    case .categoryMismatch: return "Referenced entry is outside the target category."
    case .targetFieldMissing: return "Target field is no longer available."
    case .targetFieldUnsupported: return "Target field is not a text field."
    case .targetFieldEmpty: return "Target field is empty."
    case .resolved:
        guard let target = resolution.target else { return "Referenced entry is unavailable." }
        let label = target.entryLabel.isEmpty ? "Untitled" : target.entryLabel
