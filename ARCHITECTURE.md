...
- Every maintained domain layer resolves `fieldReference` in one hop with the same nine-state precedence, exact stable IDs, a legacy empty-ID field-name fallback, and a minimal target projection. Search indexes only a resolved target's label, category, and target field name; the resolved field value remains behind the explicit unlocked detail/copy boundary.
- Category renames propagate matching `targetCategory` metadata, copy imports remap only selected target entry IDs, and referenced target text fields may be renamed but not deleted or retyped. Sync conflict copies preserve the source value and both template-field IDs.
